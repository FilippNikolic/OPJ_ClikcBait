...
\title{\textbf{Tehnički vodič --- detaljna verzija}\\[2pt]
...
\noindent
Ovo je \textbf{detaljna verzija} tehničkog vodiča: prolazi kroz \emph{ceo izvorni kod}
projekta, fajl po fajl, ali sa \emph{proširenom teorijom} --- svaki pojam je objašnjen
do nivoa na kome treba da bude jasan i bez prethodnog znanja, uz konkretne primere i
brojke. Sažeta verzija je \texttt{Tehnicki-vodic-kroz-kod.pdf}; pregled „gde se šta
nalazi'' u \texttt{docs/Mapa-koda.md}, a komande za pokretanje u
...
\begin{teorija}[ --- kako tekst postaje brojevi (vektorizacija)]
Model ne razume reči, samo brojeve. \textbf{Vektorizacija} pretvara naslov u vektor:
napravi se \emph{vokabular} svih reči u korpusu, i svaki naslov se predstavi redom
brojeva --- po jedan za svaku reč iz vokabulara (koliko se puta javlja). To je
\textbf{vreća reči} (redosled se gubi, samo se broji prisustvo). Primer sa vokabularom
\{\kod{zvezda, pobedila, šok, transfer}\}:
\begin{center}\small
\kod{"Zvezda pobedila"} $\to [1,1,0,0]$ \quad\quad
\kod{"Šok transfer"} $\to [0,0,1,1]$
\end{center}
Pošto vokabular ima desetine hiljada reči, a naslov tek nekoliko, vektori su
\emph{ogromni i skoro svi nule} (\emph{retki}/sparse). Zato su važni i normalizacija
(da oblika bude manje) i \kod{min\_df} (da retke reči ne naduvaju vokabular).
\end{teorija}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{TF-IDF} = TF $\times$ IDF --- spaja oba: visoko vrednuje reč koja je
    ...
    važnije od jedne.
\end{itemize}
\textbf{Mali primer IDF-a:} u korpusu od 2200 naslova reč „je'' se javlja u
$\approx$2000, a „transfer'' u 40. Onda $\mathrm{IDF}(\text{je})=\log(2200/2000)\approx
0{,}1$, a $\mathrm{IDF}(\text{transfer})=\log(2200/40)\approx 4{,}0$ --- „transfer''
dobija $\sim$40$\times$ veću težinu jer je informativniji.
...
i sprečavaju preprilagođavanje). \textbf{N-grami:} unigram (1--1) gleda reči pojedinačno;
bigram (1--2) dodaje i parove susednih reči --- npr.\ „velika'' i „vest'' zasebno su
neutralne, ali bigram „velika vest'' je signal. Zato 1--2 ume da uhvati fraze koje
vreća pojedinačnih reči promaši.
\end{teorija}
...
\textbf{Problem koji rešava:} ako bismo testirali na jednom fiksnom delu podataka,
rezultat bi zavisio od sreće u tom razdvajanju. \textbf{Unakrsna validacija (CV)} deli
skup na $k$ jednakih delova (fold-ova); $k$ puta trenira na $k\!-\!1$ dela i testira na
preostalom (svaki deo jednom bude test), pa usrednji --- tako se meri na \emph{svim}
podacima. Mi koristimo $k{=}10$ (kompromis: dovoljno trening podataka po fold-u, a
ocena stabilna).
\textbf{Stratifikovana} = svaki fold čuva odnos klasa 50/50; inače bi neki fold mogao
slučajno dobiti npr.\ 70\,\% klikbejta i iskriviti metriku.
\textbf{Zašto \emph{ugnežđena} (nested):} hiperparametre treba izabrati \emph{ne gledajući}
podatke na kojima prijavljujemo rezultat. Zato dve petlje: \emph{unutrašnja} bira
\kod{C}/\kod{alpha} samo unutar trening dela, a \emph{spoljna} ocenjuje na fold-u koji
pri tom izboru nije korišćen. Da koristimo jednu petlju za oboje, „provirili'' bismo u
test $\Rightarrow$ lažno bolji (optimistično pristrasan) rezultat. \kod{random\_state=42}
čini podele ponovljivim (svako dobije iste fold-ove).
\end{teorija}
...
\medskip
\textbf{Kako prepoznati pravu vrednost --- over/underfitting.} Overfitting (preveliko
$C$): model je odličan na trening podacima, ali slab na test --- „napamet'' je naučio,
ne generalizuje. Underfitting (premalo $C$): slab i na trening i na test --- premoćna
kazna ga je učinila pretupavim. Idealni hiperparametar je u sredini, gde je test
rezultat najveći --- a upravo to bira unutrašnja validacija.
\end{teorija}
...
Svaka predikcija pada u jedno od četiri polja \emph{matrice konfuzije}: \textbf{TP}
(tačno pogođen klikbejt), \textbf{FP} (regularan pogrešno proglašen klikbejtom, „lažna
uzbuna''), \textbf{FN} (klikbejt promašen), \textbf{TN} (tačno pogođen regularan). Iz
njih:
\[
\text{preciznost}=\frac{TP}{TP+FP}, \qquad \text{odziv}=\frac{TP}{TP+FN}.
\]
\textbf{Preciznost} = od svih koje je model nazvao klikbejtom, koliko ih je stvarno to.
\textbf{Odziv} (\emph{recall}) = od svih stvarnih klikbejtova, koliko ih je uhvatio.
Njih dvoje su u tenziji: ako model agresivno svuda viče „klikbejt'', odziv raste ali
preciznost pada (mnogo FP). \textbf{Primer:} od 100 stvarnih klikbejtova model uhvati
70 ($TP{=}70, FN{=}30 \Rightarrow$ odziv 0,70), ali uz to 30 regularnih proglasi
klikbejtom ($FP{=}30 \Rightarrow$ preciznost $70/100=0{,}70$). \textbf{F1} je
harmonijska sredina preciznosti i odziva --- jedan broj koji je visok samo ako su
\emph{oba} visoka (kažnjava disbalans jače od obične sredine). \textbf{Zašto F1 baš na
klikbejt klasi:} nju želimo da detektujemo; sama tačnost bi bila varljiva (model koji
sve proglasi regularnim ima 50\,\% tačnosti na balansiranom skupu, a nula korisnosti).
\textbf{Makro-F1} usrednji F1 obe klase ravnopravno. \textbf{ROC-AUC} meri koliko dobro
model \emph{rangira} naslove po verovatnoći, nezavisno od praga 0,5: 1,0 savršeno, 0,5
nasumično --- imaju ga samo modeli koji daju verovatnoću (NB, LR, BERT), a ne dekoder
sa tvrdom 0/1 odlukom.
\end{teorija}
...
\emph{pažnje} (\emph{attention}) za svaku reč gleda \emph{kontekst} (ostale reči).
Intuicija pažnje: pri obradi reči „šok'', model „obrati pažnju'' na okolne reči da
proceni da li je u pitanju senzacionalizam (\kod{„šok u Humskoj''}) ili stvarni
sadržaj (\kod{„šok terapija posle povrede''}) --- ista reč, različito značenje. Otud
„kontekstualni'' modeli: ista reč dobija različitu reprezentaciju zavisno od okoline,
za razliku od vreće reči koja je slepa za kontekst (zato baseline teško hvata suptilni
klikbejt). \textbf{Enkoder} (BERT) je predtreniran da \emph{razume} tekst --- uči tako
što se zamaskira reč i model je pogađa gledajući kontekst \emph{i levo i desno};
idealan za klasifikaciju. \textbf{Dekoder} (GPT, Claude) je predtreniran da
\emph{generiše} --- predviđa sledeću reč gledajući samo unazad; koristimo ga u Fazi 3c.
\textbf{Fine-tuning} = uzmemo model već \emph{predtreniran} na milijardama reči
(opšte znanje jezika) i kratko ga doučimo na našem malom označenom skupu, da to znanje
usmerimo baš na klikbejt --- mnogo jeftinije i bolje nego učiti od nule. \textbf{Mono
vs multi:} BERTić je treniran samo na srpskom i srodnim jezicima, pa sav kapacitet
troši na njih; mBERT isti kapacitet deli na 100+ jezika --- zato BERTić bolje hvata
nijanse i pobeđuje.
\end{teorija}
